...
import {
    Box, List, ListItem, ListItemButton, ListItemText, ListItemAvatar,
    Avatar, Typography, IconButton, TextField, Button,
    Dialog, DialogTitle, DialogContent, DialogActions, Chip, CircularProgress,
    Alert
} from '@mui/material';
import AddIcon from '@mui/icons-material/Add';
...
const ChatList = ({ onSelectChat, selectedChatId }) => {
    ...
    const getChatAvatar = (chat) => {
        if (chat.type === 'GROUP') {
            return { type: 'group', name: chat.name };
        }
        const otherUser = chat.participants?.find(p => p.id !== user?.id);
        return { type: 'private', user: otherUser };
    };

    const getAvatarComponent = (chat) => {
        ...
        // Приватный чат — фото пользователя или инициалы
        const userData = avatar.user;
        const avatarUrl = userData?.avatarUrl;
        const initials = userData?.fullName?.[0] || userData?.firstName?.[0] || '👥';
                    ...
                    <List>
                        {users.map(u => (
                            <ListItem key={u.id} disablePadding>
                                <ListItemButton onClick={() => handleCreatePrivateChat(u.id)} disabled={creating}>
                                    <ListItemAvatar>
                                        {u.avatarUrl ? (
                                            <Avatar src={`http://localhost:8080${u.avatarUrl}`} />
                                        ) : (
                        ...
                        {users.map(u => (
                            <ListItem key={u.id} disablePadding>
                                <ListItemButton onClick={() => toggleUser(u.id)}>
                                    ...
                                    <ListItemText primary={`${u.firstName} ${u.lastName}`} secondary={u.email} />
                                    {selectedUsers.includes(u.id) && <Chip label="Выбран" size="small" color="primary" />}
                                </ListItemButton>
                            </ListItem>
                        ))}
                    </List>
